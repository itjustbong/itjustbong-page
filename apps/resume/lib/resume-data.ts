...
export const personalProjects: Project[] = [
  ...
  },
  {
    id: "log-itjustbong",
    name: "log.itjustbong",
    description: "`MDX` 기반 정적 테크 블로그",
    period: "2025.12 ~",
    url: "https://log.itjustbong.com",
    highlights: [
      "**DB 없는 콘텐츠 관리**: `Next.js` + `MDX` 기반 파일 시스템 콘텐츠 관리, 정적 빌드 및 *Vercel 무료 플랜 운영*",
      "**SEO 최적화**: sitemap, robots, JSON-LD, 동적 OG 이미지, `Mermaid` 다이어그램 지원",
      "**모노레포 플랫폼 확장** 및 MFA Ready: `Turborepo` + `pnpm` 기반 apps(blog/resume/llm/shell) + packages(ui/shared/config) 구조로 여러 Next.js 앱 간 *재사용성 및 개발 효율 향상*",
    ],
