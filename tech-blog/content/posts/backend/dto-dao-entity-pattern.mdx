---
title: "[Backend] DTO, DAO, Entity 패턴 이해하기"
description: "백엔드 아키텍처의 핵심 패턴인 DTO, DAO, Entity의 개념과 실무 적용 방법"
category: "backend"
...
flowchart LR
    ...
    subgraph Repository["💾 Repository/DAO"]
        ACCESS["데이터 접근"]
    end

    subgraph Database["🗄️ Database"]
        ENTITY["Entity<br/>(테이블 매핑)"]
    end
...
flowchart LR
    ...
    subgraph DB["데이터베이스"]
        TABLE["users 테이블<br/>├─ id (PK)<br/>├─ name<br/>└─ email"]
    end
...
```mermaid
flowchart TB
    subgraph Entity["Entity (DB 레이어)"]
        E1["User"]
        E2["Post"]
        E1 -->|"1:N"| E2
    end

    subgraph DTO["DTO (API 레이어)"]
        D1["UserResponseDto"]
...
```mermaid
flowchart LR
    subgraph Entity차이["Entity vs VO"]
        E["Entity<br/>ID로 비교<br/>user1.id === user2.id"]
        V["VO<br/>값으로 비교<br/>money1.amount === money2.amount"]
    end
...
1. **DTO**: 데이터 전송용 객체 (로직 없음)
2. **DAO/Repository**: DB 접근 캡슐화 (Prisma가 대체)
3. **Entity**: DB 테이블 매핑 (ID로 식별)
4. **VO**: 값 객체 (불변, 값으로 비교)
5. **BO**: 비즈니스 로직 포함

NestJS + Prisma 조합에서는:
- **Request DTO**: class + 데코레이터 (유효성 검사)
- **Response DTO**: interface (프론트와 공유 가능)
- **Entity**: Prisma가 자동 생성
- **DAO**: Prisma Client가 담당

이 패턴들을 잘 활용하면 레이어 간 책임이 명확해지고, 테스트하기 쉬운 코드를 작성할 수 있습니다!

다음 글에서는 모노레포에서 타입을 공유하는 방법에 대해 다뤄보겠습니다.
