---
title: "[Backend] 모노레포에서 타입 공유하기: @repo/types 패키지 활용"
description: "pnpm workspace 모노레포에서 프론트엔드와 백엔드 간 타입을 효율적으로 공유하는 방법"
category: "backend"
...
# 모노레포에서 타입 공유하기: @repo/types 패키지 활용

모노레포에서 프론트엔드와 백엔드를 함께 관리하면, **타입을 공유**할 수 있다는 큰 장점이 있습니다. 하지만 NestJS를 사용하면 몇 가지 제약이 있습니다. 이 글에서는 이 문제를 어떻게 해결하는지 알아보겠습니다.
...
flowchart LR
    ...
    subgraph NestJS["NestJS 요구사항"]
        C["class CreateUserDto<br/>+ @IsString()<br/>+ @IsEmail()"]
    end

    SharedTypes -->|"❌ 호환 안됨"| NestJS
...
```mermaid
flowchart TB
    subgraph Request["📥 Request (클라이언트 → 서버)"]
        R1["BE에서 class로 정의"]
        R2["유효성 검사 데코레이터 필요"]
        R3["공유 타입 ❌ 사용 어려움"]
    end

    subgraph Response["📤 Response (서버 → 클라이언트)"]
        S1["공유 타입 ✅ 사용 가능"]
        S2["interface로 충분"]
        S3["FE/BE 동일 타입 보장"]
    end
...
sequenceDiagram
    ...
    Note over FE,Types: Request 흐름
    FE->>FE: CreateUserInput (자체 타입)
    FE->>BE: POST /api/users (JSON)
    BE->>BE: CreateUserDto (class + 데코레이터)
    BE->>BE: ValidationPipe 검증

    Note over FE,Types: Response 흐름
    BE->>Types: UserResponse 타입 import
    BE->>FE: Response (JSON)
    FE->>Types: UserResponse 타입 import
    FE->>FE: 타입 안전하게 사용
```
...
```mermaid
flowchart LR
    subgraph Copy["Docker COPY 필수 파일"]
        A["pnpm-lock.yaml"]
...
```mermaid
flowchart TB
    subgraph Shared["📦 @repo/shared-types"]
        direction LR
        R["Response 타입"]
        E["Enum"]
        C["상수"]
    ...
    subgraph Backend["🔧 Backend"]
        ...
        RES["Response<br/>(공유 타입 사용)"]
    end

    subgraph Frontend["🖥️ Frontend"]
        INPUT["Input 타입<br/>(자체 정의 or 공유)"]
        OUTPUT["Response<br/>(공유 타입 사용)"]
    end
...
1. **Response 타입**: 공유 패키지에서 interface로 정의
2. **Request DTO**: 백엔드에서 class + 데코레이터로 별도 정의
3. **Enum/상수**: 공유 패키지에서 정의하여 양쪽에서 사용
4. **Docker 빌드**: 내부 패키지 의존성 파일 복사 필수

NestJS의 데코레이터 제약 때문에 Request DTO는 공유하기 어렵지만, Response 타입과 공통 Enum만 공유해도 **타입 안전성**과 **API 계약**을 크게 개선할 수 있습니다.

특히 Response 타입을 공유하면, 백엔드에서 응답 구조가 변경될 때 프론트엔드에서도 즉시 타입 에러가 발생하여 버그를 사전에 방지할 수 있습니다!
