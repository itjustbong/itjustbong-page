...
Dockerfile을 작성할 때 가장 중요한 두 가지가 있습니다. 바로 **멀티스테이지 빌드**와 **레이어 캐싱**입니다. 이 두 가지를 잘 활용하면 이미지 크기는 1/4로 줄이고, 빌드 시간은 5배 단축할 수 있습니다.
...
Dockerfile 최적화의 핵심을 정리하면:

1. **멀티스테이지 빌드**: Builder와 Runner를 분리하여 이미지 크기 최소화
2. **레이어 캐싱**: 변경이 적은 파일을 먼저 복사하여 빌드 시간 단축
3. **보안**: non-root 사용자 사용, 환경변수는 런타임에 주입

이 세 가지만 지키면 프로덕션에서도 안심하고 사용할 수 있는 Dockerfile을 만들 수 있습니다.

다음 글에서는 Prisma와 Docker를 함께 사용할 때 자주 만나는 바이너리 타겟 문제에 대해 다뤄보겠습니다!
