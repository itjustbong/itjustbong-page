---
title: "[Docker] Docker Compose 완벽 가이드: 네트워크, 볼륨, 포트 매핑"
description: "Docker Compose의 핵심 개념인 네트워크, 볼륨, 포트 매핑을 실무 예제와 함께 이해하기"
category: "devops"
...
# Docker Compose 완벽 가이드: 네트워크, 볼륨, 포트 매핑

Docker Compose를 사용하면서 가장 헷갈리는 부분이 바로 **네트워크**, **볼륨**, **포트 매핑**입니다. 이 글에서는 이 세 가지 핵심 개념을 실무 예제와 함께 명확하게 정리해보겠습니다.
...
> 💡 **기억하세요**: 컨테이너 내부 포트(오른쪽)는 앱이 실제로 리슨하는 포트와 같아야 합니다. 변경할 수 있는 건 호스트 포트(왼쪽)뿐입니다.
...
flowchart TB
    subgraph Network["🌐 Docker Network (app-network)"]
        ...
        subgraph DB["PostgreSQL 컨테이너"]
            DB_APP[PostgreSQL]
        end
        subgraph RD["Redis 컨테이너"]
            RD_APP[Redis]
        end
    end
...
flowchart LR
    ...
    subgraph 용도
        ...
        PS["docker ps에서 식별용<br/>my-redis"]
    end
...
```mermaid
flowchart TB
    subgraph Drivers["Network Driver 종류"]
        direction LR
        B[bridge<br/>기본값]
        H[host<br/>최고 성능]
        O[overlay<br/>멀티 호스트]
        M[macvlan<br/>물리 네트워크]
        N[none<br/>완전 격리]
    end

    B --> B_DESC["단일 호스트 내 가상 스위치<br/>컨테이너 간 통신에 사용"]
    H --> H_DESC["호스트 OS 네트워크 직접 사용<br/>포트 매핑 불필요"]
    O --> O_DESC["여러 서버 간 네트워크 연결<br/>Docker Swarm/K8s 필요"]
    M --> M_DESC["물리 NIC에 직접 연결<br/>고급 사용 사례"]
    N --> N_DESC["네트워크 없음<br/>보안 격리용"]
...
flowchart LR
    subgraph 볼륨없음["❌ 볼륨 없이"]
        C1[컨테이너 생성] --> D1[데이터 저장] --> R1[컨테이너 삭제] --> L1[💀 데이터 손실!]
    end

    subgraph 볼륨있음["✅ 볼륨 사용"]
        C2[컨테이너 생성] --> V2[볼륨에 저장] --> R2[컨테이너 삭제] --> S2[✅ 데이터 유지!]
    end

    style 볼륨없음 fill:#FFE4E1
    style 볼륨있음 fill:#E8F8E8
```
...
flowchart TB
    subgraph NamedVolume["📦 Named Volume"]
        ...
        NV_LOC["실제 위치: /var/lib/docker/volumes/"]
        NV_MGT["Docker가 자동 관리"]
    end

    subgraph BindMount["📁 Bind Mount"]
        BM_DEF["./data:/data"]
        BM_LOC["실제 위치: 호스트의 ./data"]
        BM_MGT["개발자가 직접 관리"]
    end
...
Docker Compose의 핵심 개념을 정리해보면:

1. **포트 매핑**: `호스트:컨테이너` 형식으로 외부 접근 경로 설정
2. **네트워크**: 같은 네트워크의 컨테이너끼리 서비스 이름으로 통신
3. **볼륨**: 컨테이너 삭제 후에도 데이터 유지

이 세 가지만 확실히 이해하면 Docker Compose를 자유자재로 활용할 수 있습니다. 특히 **서비스 이름으로 통신**한다는 점과 **Named Volume은 하단에 정의**해야 한다는 점을 기억하세요!

다음 글에서는 Dockerfile 작성법과 멀티스테이지 빌드에 대해 다뤄보겠습니다.
