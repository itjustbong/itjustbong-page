---
title: "[Docker] Docker 기초: 이미지와 컨테이너의 관계"
description: "Docker 이미지와 컨테이너의 개념, 그리고 빌드부터 실행까지의 전체 흐름 이해하기"
category: "devops"
...
Docker를 처음 접하면 **이미지**와 **컨테이너**라는 용어가 자주 등장합니다. 비슷해 보이지만 완전히 다른 개념인데요, 이 둘의 관계를 명확히 이해하면 Docker를 훨씬 편하게 사용할 수 있습니다.
...
```mermaid
flowchart LR
    subgraph Image["📦 이미지 (Image)"]
        I[붕어빵 틀<br/>설계도<br/>클래스]
    end

    subgraph Containers["🍞 컨테이너들 (Containers)"]
        C1[붕어빵 1<br/>인스턴스 1]
        C2[붕어빵 2<br/>인스턴스 2]
        C3[붕어빵 3<br/>인스턴스 3]
    end
...
flowchart LR
    subgraph Build["🔨 빌드 단계"]
        DF[Dockerfile] -->|docker build| IMG[이미지]
    end

    subgraph Run["▶️ 실행 단계"]
        IMG -->|docker run| CON[컨테이너]
    end

    subgraph Registry["☁️ 레지스트리"]
        HUB[(Docker Hub)]
    end
...
```mermaid
flowchart TB
    subgraph BuildProcess["docker build 과정"]
...
```mermaid
flowchart TB
    subgraph Image["📦 이미지: my-app:1.0"]
        IMG[애플리케이션<br/>+ 의존성<br/>+ 설정]
    end

    subgraph Containers["실행 중인 컨테이너들"]
        C1["🟢 my-app-1<br/>포트: 3001"]
        C2["🟢 my-app-2<br/>포트: 3002"]
        C3["🟢 my-app-3<br/>포트: 3003"]
    end
...
flowchart LR
    subgraph Compose["docker compose up --build"]
        A["docker-compose.yml"] --> B["1. docker build<br/>(이미지 생성)"]
        B --> C["2. docker run<br/>(컨테이너 실행)"]
    end
...
flowchart LR
    ...
    E1 --> E1_D["공식 이미지, alpine 태그"]
    E2 --> E2_D["공식 이미지, 버전 7"]
    E3 --> E3_D["로컬 이미지, 버전 1.0"]
    E4 --> E4_D["GitHub 레지스트리, latest 태그"]
```
...
1. **이미지** = 불변의 템플릿 (설계도, 클래스)
2. **컨테이너** = 실행 중인 인스턴스 (제품, 객체)
3. **docker build** = 이미지 생성 (RUN 실행)
4. **docker run** = 컨테이너 실행 (CMD 실행)

이 개념만 확실히 이해하면 Docker의 나머지 기능들도 쉽게 이해할 수 있습니다. 특히 이미지와 컨테이너의 관계를 **클래스와 인스턴스**로 생각하면 훨씬 직관적으로 이해됩니다!

다음 글에서는 Docker에서 Prisma를 사용할 때 자주 만나는 바이너리 타겟 문제에 대해 다뤄보겠습니다.
