...
# Turborepo 태스크 설정하기 (Configuring Tasks)

태스크(Task)는 Turborepo가 실행하는 스크립트입니다. `turbo.json`에서 태스크 간의 관계를 정의하면, Turborepo가 최적의 실행 순서와 병렬 처리를 자동으로 결정합니다.
...
이러한 설정을 통해 Turborepo는 최적의 빌드 순서와 병렬 처리를 자동으로 결정하여 개발 생산성을 크게 향상시킵니다.

다음 글에서는 Turborepo의 캐싱 메커니즘과 원격 캐싱 설정에 대해 자세히 알아보겠습니다.
