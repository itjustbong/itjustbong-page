...
# Turborepo 애플리케이션 개발하기 (Developing Applications)

모노레포에서 애플리케이션을 개발하는 것은 각 소스 컨트롤의 아토믹한 커밋을 가능하게 합니다. 대부분의 개발은 코드를 수정하는 긴 작업이며, Turborepo는 터미널 UI와 다양한 기능으로 개발 경험을 개선합니다.

이 글에서는 Turborepo를 활용한 효율적인 개발 워크플로우를 알아보겠습니다.
...
Turborepo 개발 워크플로우의 핵심:

1. **개발 태스크 설정**
   - `cache: false`: 캐싱 비활성화
   - `persistent: true`: 장시간 실행

2. **의존성 관리**
   - `dependsOn`: 설정 작업 자동 실행
   - 필터로 특정 앱만 실행
...
- [ ] `dev` 태스크에 `persistent: true` 설정
- [ ] 필요한 설정 작업을 `dependsOn`에 추가
- [ ] 각 앱마다 다른 포트 사용
- [ ] 필터로 필요한 패키지만 실행
- [ ] Watch mode로 자동 재실행 활용
- [ ] 터미널 UI 단축키 숙지

Turborepo의 강력한 개발 도구를 활용하면 모노레포에서도 빠르고 효율적인 개발이 가능합니다. 특히 watch mode와 터미널 UI를 잘 활용하면 생산성을 크게 향상시킬 수 있습니다.

이것으로 Turborepo 시리즈를 마칩니다. 모노레포 개발에 도움이 되길 바랍니다!
