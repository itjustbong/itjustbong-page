...
---
 ...
 Turborepo 시작하기 (Getting Started)

Turborepo는 JavaScript/TypeScript 모노레포를 위한 고성능 빌드 시스템입니다. 이 글에서는 Turborepo를 처음 시작하는 방법부터 기존 프로젝트에 적용하는 방법까지 단계별로 알아보겠습니다.

## 새 프로젝트 시작하기

### 1. 초기 템플릿 설치

가장 빠르게 시작하는 방법은 공식 템플릿을 사용하는 것입니다.
...
코드를 변경하지 않고 동일한 명령어를 다시 실행해보세요.
...
더 강력한 자동완성과 린팅 기능을 원한다면 공식 VS Code 확장 프로그램을 설치하세요.
...
Turborepo의 기본 사용법을 익혔다면, 다음 주제들을 살펴보세요:

- **캐싱 전략**: 원격 캐싱과 로컬 캐싱 최적화
- **파이프라인 구성**: 복잡한 태스크 의존성 관리
- **필터링**: 특정 패키지나 변경사항만 대상으로 실행
- **CI/CD 통합**: GitHub Actions, Vercel 등과 연동

Turborepo는 모노레포의 빌드 속도를 획기적으로 개선할 수 있는 강력한 도구입니다. 캐싱과 병렬 실행을 통해 개발 생산성을 크게 향상시킬 수 있습니다.
