...
# Turborepo 의존성 관리하기 (Managing Dependencies)

모노레포에서 의존성을 어떻게 관리하느냐에 따라 프로젝트의 유지보수성과 빌드 성능이 크게 달라집니다. 이 글에서는 Turborepo 환경에서 의존성을 효과적으로 관리하는 방법을 알아보겠습니다.
...
모노레포에서 의존성을 올바르게 관리하고 있는지 확인해보세요.

- [ ] 각 패키지가 사용하는 의존성을 해당 패키지의 `package.json`에 명시했나요?
- [ ] 루트 `package.json`에는 레포지토리 관리 도구만 설치했나요?
- [ ] 동일한 의존성의 버전이 패키지마다 다르지 않나요?
- [ ] 내부 패키지 의존성을 `workspace:*` 프로토콜로 명시했나요?
- [ ] pnpm Catalog나 syncpack 같은 도구로 버전을 관리하고 있나요?

## 정리

Turborepo 환경에서 의존성 관리의 핵심 원칙:

1. **사용하는 곳에 설치**: 각 패키지가 필요한 의존성만 명시
2. **루트는 최소화**: 레포지토리 관리 도구만 루트에 설치
3. **버전 통일**: Catalog나 전용 도구로 버전 일관성 유지
4. **역할 구분 이해**: Turborepo는 빌드 관리, 패키지 매니저는 의존성 관리
5. **내부 의존성 명시**: `workspace:*`로 워크스페이스 패키지 연결

이러한 원칙을 따르면 각 패키지가 독립적으로 동작하면서도, 전체 모노레포가 일관되게 관리됩니다. 또한 Turborepo의 캐싱 기능을 최대한 활용할 수 있습니다.

다음 글에서는 `turbo.json`을 통한 태스크 설정과 의존성 기반 빌드 파이프라인 구성에 대해 알아보겠습니다.
