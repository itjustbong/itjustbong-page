...
이전 글에서 JavaScript 모듈 시스템의 역사를 살펴봤다면, 이번에는 **CommonJS(CJS)** 의 내부 동작 원리를 깊이 있게 파헤쳐보겠습니다. "어떻게 `require()`가 작동하는가?"라는 질문에 답할 수 있게 될 것입니다.
...
`require()` 함수를 호출하면 내부적으로 5단계의 프로세스가 진행됩니다. 각 단계를 자세히 살펴보겠습니다.
...
CommonJS의 동작 원리를 이해하면:

- `require()`가 단순한 함수 호출이 아니라 복잡한 5단계 프로세스임을 알 수 있습니다
- 모듈 캐싱이 성능과 싱글톤 패턴에 어떻게 기여하는지 이해할 수 있습니다
- 순환 참조 문제를 예방하고 해결할 수 있습니다
- NPM 패키지가 어떻게 구성되는지 파악할 수 있습니다

다음 글에서는 ESM(ECMAScript Module)의 동작 원리와 CommonJS와의 차이점을 비교해보겠습니다!
