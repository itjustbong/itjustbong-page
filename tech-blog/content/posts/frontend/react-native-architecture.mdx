---
title: "React Native 앱 구조와 작동 원리"
description: "React Native 앱의 두 레이어 구조와 JS-Native 통신 원리, Bridge/JSI 아키텍처에 대해 알아봅니다"
category: "frontend"
...
React Native로 앱을 개발할 때 가장 중요한 것은 앱의 내부 구조를 이해하는 것입니다. 이 문서에서는 React Native 앱이 어떻게 구성되어 있고, JavaScript와 네이티브 코드가 어떻게 공존하는지 자세히 설명합니다.
...
React Native 앱은 **네이티브 Shell**과 **JavaScript 번들**이 **Bridge(또는 JSI)**를 통해 통신하는 구조입니다. 이 구조를 이해하면:

- 왜 Hot Reload가 가능한지
- 왜 네이티브 라이브러리 추가 시 재빌드가 필요한지
- 왜 성능 이슈가 발생할 수 있는지
