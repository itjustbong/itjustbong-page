---
title: "Expo 네이티브 폴더 생성 시점과 구조"
description: "ios/, android/ 폴더가 언제 생성되고, 어떤 구조로 이루어져 있는지 알아봅니다"
category: "frontend"
...
많은 분들이 헷갈려하는 부분 중 하나가 네이티브 폴더(`ios/`, `android/`)의 생성 시점입니다. 이 문서에서는 네이티브 폴더가 언제 생성되고, 어떤 구조로 이루어져 있으며, 어떻게 수정하는지 자세히 설명합니다.
...
1. **로컬 빌드** 시에만 `ios/`, `android/` 폴더가 로컬에 생성됨
2. **EAS 빌드**는 클라우드에서 빌드하므로 로컬 폴더 불필요
3. **직접 수정** 시 `prebuild --clean`에 주의
4. **Config Plugin** 사용이 권장됨
